...
# Install Docker CLI and PostgreSQL 18 client for container lifecycle management and database dumps
RUN apt-get update && apt-get install -y --no-install-recommends \
    ...
    apt-get update && \
    apt-get install -y --no-install-recommends docker-ce-cli postgresql-client-18 && \
    apt-get clean && \
