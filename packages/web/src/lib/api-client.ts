...
const apiFetch = async <T>(path: string, init?: RequestInit): Promise<T> => {
  ...
  if (!response.ok) {
    ...
    throw new Error(body?.error ?? body?.message ?? `API error ${String(response.status)}`)
  }
...
const deleteSource = (id: string) => apiFetch<undefined>(`/api/sources/${id}`, { method: "DELETE" })
...
const deleteDestination = (id: string) => apiFetch<undefined>(`/api/destinations/${id}`, { method: "DELETE" })
...
const fetchRetention = () => apiFetch<{ globalRetention: number }>("/api/retention")
...
const fetchNotifications = () => apiFetch<{ discord?: { webhookUrl: string; enabled: boolean } }>("/api/notifications")
...
const fetchBackups = (page = 1, limit = 50) =>
  apiFetch<{ data: BackupRecord[]; total: number; page: number; limit: number }>(
    `/api/backups?page=${String(page)}&limit=${String(limit)}`,
  )
...
const triggerBackup = () => apiFetch<{ success: boolean; message: string }>("/api/backups/run", { method: "POST" })

const fetchBackupProgress = () => apiFetch<BackupRunProgress>("/api/backups/progress")
...
const fetchLogs = (level?: string, search?: string, limit = 100) => {
  ...
  return apiFetch<{ data: { id: string; timestamp: string; level: string; message: string }[]; total: number }>(
...
const fetchDockerContainers = () => apiFetch<string[]>("/api/sources/containers")
...
const fetchPostgresDatabases = (data: {
